DROP PROCEDURE IF EXISTS generate_os_version_report;
DELIMITER //

CREATE PROCEDURE generate_os_version_report(start_date date, end_date date)
BEGIN
select 
	A.os_version, 
    A.offer_name,
...
(
    ...
    group by os_version, offer_name    
) as A
left join 
(
	select 
...
on A.os_version = B.os_version and A.offer_name = B.offer_name
left join
(
	select 
		os_version, 
...
on A.os_version = C.os_version and A.offer_name = C.offer_name
left join
(
	select 
		os_version, 
...
on A.os_version = D.os_version and A.offer_name = D.offer_name;
END
