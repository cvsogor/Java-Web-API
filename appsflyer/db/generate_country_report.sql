DROP PROCEDURE IF EXISTS generate_country_report;
DELIMITER //

CREATE PROCEDURE generate_country_report(start_date date, end_date date)
BEGIN
select 
	A.country_code, 
    A.offer_name,
...
on A.country_code = B.country_code and A.offer_name = B.offer_name
left join
(
	select 
		country_code, 
...
on A.country_code = C.country_code and A.offer_name = C.offer_name
left join
(
	select 
		country_code, 
...
on A.country_code = D.country_code and A.offer_name = D.offer_name;
END
